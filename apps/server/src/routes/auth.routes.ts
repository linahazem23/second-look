...
// Not verified by an SMS code (no provider wired up — same tradeoff as the
// guardian's phone) — just a real contact number so Second Look can reach a
// member directly and fast for anything urgent.
authRouter.patch('/phone', requireAuth, async (req: AuthedRequest, res) => {
  ...
  const user = await prisma.user.update({ where: { id: req.userId }, data: { phoneNumber: parsed.data.phoneNumber } });
